//Q. 1. (b) Newton backward interpolation
...
void backward(double *, double, int); //carries out the actual interpolation
...
{
	int i, j;
	double *table = (double*)calloc(sizeof(double), n*(n+1));
	printf("x\ty");
	for(i=0;i<n;i++)
	{
		*(table+i) = *(p+i);
...
void backward(double *table, double x, int n)
{
	int i, j, k;
	double nr, dr, s, h = (*(table+1) - *table), result = *(table+(2*n)-1);
	for(i=1;i<n;i++)
	{
		s = (x-*(table+n-1))/h;
		dr=1;
		k=1;
		nr = *(table+(n*(i+1))+(n-i-1));
		for(j=0; j<i; j++)
		{
			nr *= s;
			dr *= k;
